...
unit = pd.read_csv('csv_files/north_carolina/nc18unit.csv')
person = pd.read_csv('csv_files/north_carolina/nc18per.csv')
crash = pd.read_csv('csv_files/north_carolina/nc18crash.csv')
road = pd.read_csv('csv_files/north_carolina/nc18road.csv')


#for unit, drop: OID_, PEDCONT1, PEDCONT2, DRV_INJ, DRV_REST, IMPACTSP, MOSTHARM, PHYSCOND, MANEUVER, CONTRIB1, CONTRIB2, CONTRIB3
unit = unit.drop(['OID_', 'PEDCONT1', 'PEDCONT2', 'DRV_INJ', 'DRV_REST', 'IMPACTSP', 'MOSTHARM', 'PHYSCOND', 'MANEUVER','ALCFLAG'], axis=1)

#only include PEDFLAG = 'N'
unit = unit[unit['PEDFLAG'] == 'N']
print("pedflag")
...
#add a column called DISTRACTED that is 1 if CONTRIB1 (or 2 or 3) equal to 23, 35, 36, 37, 38
crash_unit['DISTRACTED'] = 0
...
for index, row in crash_unit.iterrows():
    ...
    if(row['CONTRIB1'] == 23 or row['CONTRIB1'] == 35 or row['CONTRIB1'] == 36 or row['CONTRIB1'] == 37 or row['CONTRIB1'] == 38):
        crash_unit.at[index, 'DISTRACTED'] = 1
    elif(row['CONTRIB2'] == 23 or row['CONTRIB2'] == 35 or row['CONTRIB2'] == 36 or row['CONTRIB2'] == 37 or row['CONTRIB2'] == 38):
        crash_unit.at[index, 'DISTRACTED'] = 1
    elif(row['CONTRIB3'] == 23 or row['CONTRIB3'] == 35 or row['CONTRIB3'] == 36 or row['CONTRIB3'] == 37 or row['CONTRIB3'] == 38):
        crash_unit.at[index, 'DISTRACTED'] = 1
...
#save to csv
#crash_unit.to_csv('csv_files/modeling/nc18crash_unit.csv', index=False)
...
df.to_csv('csv_files/modeling/nc18distracted_driving_prelim.csv', index=False)
...
df.to_csv('csv_files/modeling/nc18distracted_driving.csv', index=False)